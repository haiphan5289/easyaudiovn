...
public extension UITextField {
    ...
    func addCustomBottomLine(marginLeft : CGFloat = 0, marginRight : CGFloat = 0) {
        let view = UIView.init()
        view.backgroundColor = .lightGray
        self.addSubview(view)
        view.snp.makeConstraints { (make) in
            make.leftMargin.equalToSuperview().offset(marginLeft)
            make.rightMargin.equalToSuperview().offset(marginRight)
