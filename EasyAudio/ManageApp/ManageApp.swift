...
class ManageApp {
    ...
    @VariableReplay var pinHomes: [FolderModel] = AppSettings.pinFolders
    var refreshValue: PublishSubject<Void> = PublishSubject.init()
    ...
    func start() {
        self.setupRX()
        self.createFolder(path: ConstantApp.shared.folderPhotos, success: nil, failure: nil)
        self.createFolder(path: ConstantApp.shared.folderVideos, success: nil, failure: nil)
        self.createFolder(path: ConstantApp.shared.folderMusics, success: nil, failure: nil)
        self.createFolder(path: ConstantApp.shared.folderTrashs, success: nil, failure: nil)
        self.createFolder(path: ConstantApp.shared.folderDocuments, success: nil, failure: nil)
        
        self.defaultValueFolders()
    }
    ...
    private func setupRX() {
        ...
        Observable.merge(getList, updateList, deleteList, refreshValue).bind { [weak self] list in
            ...
            file += wSelf.getDirectory()
            filesHome += wSelf.getDirectory()
            wSelf.foldersRoot = wSelf.getFoldersRoot()
            ...
            wSelf.files = file
            wSelf.filesHome = filesHome
        }.disposed(by: disposeBag)
        
        self.$pinHomes.asObservable().bind { list in
            AppSettings.pinFolders = list
    ...
    func sortDatasource(folders: [FolderModel], sort: SortModel) -> [FolderModel] {
        switch sort.sort {
        case .name:
    ...
    private func defaultValueFolders() {
        if AppSettings.isFirstApp {
...
//    }
    ...
    func write(text: String, nameFile: String) async throws -> Result<URL, Error> {
...
//        try? text.write(to: file, atomically: false, encoding: String.Encoding.utf8)
    ...
    func addPinFolder(folder: FolderModel) {
...
//    }
    ...
    func fetchImage(image: UIImage) async throws -> Result<URL, Error> {
        guard let data = image.jpegData(compressionQuality: 1) ?? image.pngData() else {
        ...
        let photosPath = documentDirectoryPath.appendingPathComponent(ConstantApp.shared.folderPhotos)
    ...
    func savePdf(data: Data, fileName: String) async throws -> Result<URL, Error> {
    ...
    func createURL(folder: String, name: String) -> URL {
...
//            }
    ...
    func getNameOrigin(string: String) -> String {
    ...
    func removePreviousFolder(url: URL) -> String {
    ...
    //MARK: CUT THE PREVIOUS FOLDER
    func getNameFolderToCompress(url: URL) -> String {
        let att = NSMutableAttributedString(string: self.detectPathFolder(url: url))
        let list = self.rangeTexts(source: att, searchText: "/")
        if list.count <= 1 {
            return ""
    ...
    func cutThePreviousFolder(url: URL) -> String {
        let att = NSMutableAttributedString(string: self.detectPathFolder(url: url))
        let list = self.rangeTexts(source: att, searchText: "/")
        let last = list.last
        let start = att.string.index(att.string.startIndex, offsetBy: 0)
    ...
    //MARK: GET URL FOLDER
    func getUrlFolder(folder: String) -> URL {
        guard let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first else {
    ...
    func getfolders(url: URL) -> [URL] {
        do {
            ...
            let subdirs = directoryContents.filter{ $0.hasDirectoryPath }
                .filter{ !$0.absoluteString.contains(ConstantApp.shared.fileRealmManage) }
    ...
    func detectPathFileInFolder(url: URL) -> String {
        guard let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first else {
            return ""
        }
        let appURL = URL(fileURLWithPath: documentDirectoryPath)
    ...
    func getDirectory() -> [FolderModel] {
        ...
        if folderName.index(folderName.startIndex, offsetBy: n, limitedBy: folderName.endIndex) != nil {
            let files = self.getItemsFolderFiles(folder: folderName)
                ...
                .filter{ !$0.absoluteString.contains(ConstantApp.shared.fileRealm) }
                .filter{ !$0.absoluteString.contains(ConstantApp.shared.fileRealmManage) }
                .filter{ !$0.absoluteString.contains(ConstantApp.shared.fileRealmNote) }
                .filter{ !$0.absoluteString.contains(ConstantApp.shared.fileRealmDefault) }
                .map { url in
                    return FolderModel(imgName: nil, url: url, id: Date().convertDateToLocalTime().timeIntervalSince1970)
                }
            file += files
    ...
    func getFoldersRoot() -> [FolderModel] {
        var file: [FolderModel] = []
        guard let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first else {
            return []
        }
    ...
    func detectPathFolder(url: URL) -> String {
        guard let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first else {
            return ""
        }
        let appURL = URL(fileURLWithPath: documentDirectoryPath)
    ...
    func detectNumberofFolder(url: URL) -> Int {
        let att = NSMutableAttributedString(string: self.detectPathFolder(url: url))
        let list = self.rangeTexts(source: att, searchText: "/")
        //1 the first folder
        return list.count - 1
    ...
    func rangeTexts(source: NSMutableAttributedString, searchText: String) -> [NSRange] {
        do {
    ...
    func getItemsFolders(folder: URL) -> [URL] {
        do {
    ...
    func gettURL(folder: String) -> URL {
        guard let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first else {
    ...
    func getItemsFolderFiles(folder: String) -> [URL] {
        guard let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first else {
            return []
        }
        let appURL = URL(fileURLWithPath: documentDirectoryPath)
    ...
    func getItemsFolder(folder: String) -> [URL] {
        guard let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first else {
            return []
        }
        let appURL = URL(fileURLWithPath: documentDirectoryPath)
    ...
    func getItemsFolderCloud(cloudURL: URL) -> [URL] {
        do {
    ...
    func changeNameFile(folderName: String, oldURL: URL, newName: String, complention: ((URL) -> Void)?, failure: ((String) -> Void)?) {
        do {
    ...
    func onlyChangeFile(old: URL, new: String) async throws -> Result<URL, Error> {
        guard let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first else {
            return .failure(ErrorAsync.unknown)
        }
        let appURL = URL(fileURLWithPath: documentDirectoryPath)
    ...
    func changefile(old: String, new: String, failure: ((String) -> Void)?) {
    ...
    func changeName(old: String, new: String, failure: ((String) -> Void)?) {
    ...
    func createInFolder(imagesDirectoryPath: String, success: ((URL) -> Void)?, failure: ((String) -> Void)?) {
...
//        }
    ...
    func unZipItems(sourceURL: URL, folderName: String, failure: (String) -> Void?) {
        ...
        do {
            ...
            if let last = listName.last {
                ...
                let isFolderExist = ConstantApp.shared.folderDocuments.uppercased() == removeText.uppercased() ||
                ConstantApp.shared.folderMusics.uppercased() == removeText.uppercased() ||
                ConstantApp.shared.folderVideos.uppercased() == removeText.uppercased() ||
                ConstantApp.shared.folderPhotos.uppercased() == removeText.uppercased() ||
                ConstantApp.shared.folderTrashs.uppercased() == removeText.uppercased()
    ...
    func zipItems(sourceURL: [URL], nameZip: String) {
        do {
    ...
    //MARK: REMOVE FILES
    func removeFilesFolder(name: String) {
        guard let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first else {
            return
        }
        let fileManager = FileManager.default
    ...
    func removeFilesinFolder(name: String, listIndex: [URL]) {
        guard let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first else {
            return
        }
        let fileManager = FileManager.default
    ...
    func removeFilesFolder(folderName: String) {
        guard let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first else {
            return
        }
        let fileManager = FileManager.default
...
//    }
    ...
    func removeFolderAllfiles(url: URL) {
        let fileManager = FileManager.default
        do {
            try fileManager.removeItem(at: url)
        } catch let err {
    ...
    func covertToCAF(url: URL, completion: @escaping((URL) -> Void)) {
...
//        }
    ...
    func duplicateItem(folderName: String, at srcURL: URL) async throws -> Result<URL, Error> {
    ...
    func moveToItemText(at srcURLs: [URL], folderName: String, complention: @escaping (() -> Void), failure: @escaping ((String) -> Void)) {
        var e: Error?
    ...
    func moveToItem(at srcURLs: [URL], folderName: String, complention: @escaping (() -> Void), failure: @escaping ((String) -> Void)) {
        var e: Error?
        srcURLs.forEach { srcURL in
    ...
    func secureCopyItemstoFolder(at srcURLs: [URL], folderName: String) async throws -> Result<Void, Error> {
        var e: Error?
        srcURLs.forEach { srcURL in
            if srcURL.hasDirectoryPath {
                ...
                let id = Int(Date().convertDateToLocalTime().timeIntervalSince1970)
                let dstURL = self.createURL(folder: folderName, name: "\(id)\(srcURL.lastPathComponent)")
                do {
                    try FileManager.default.copyItem(at: srcURL, to: dstURL)
                } catch (let error) {
                   e = error
    ...
    func secureCopyItemfromiCloud(at srcURL: URL, folderName: String) async throws -> Result<URL, Error> {
        let id = Int(Date().convertDateToLocalTime().timeIntervalSince1970)
    ...
    func secureCopyItemtoFolder(at srcURL: URL, folderName: String) async throws -> Result<URL, Error> {
        let id = Int(Date().convertDateToLocalTime().timeIntervalSince1970)
    ...
    func secureCopyItemfromLibrary(at srcURL: URL, folderName: String, complention: @escaping ((URL) -> Void), failure: @escaping ((String) -> Void)) {
    ...
    func secureCopyItem(at srcURL: URL, folderName: String, complention: @escaping (() -> Void), failure: @escaping ((String) -> Void)) {
    ...
    func showActivies(urls: [URL], viewcontroller: UIViewController, complention: (() -> Void)?) {
...
//            }
    ...
    //MARK: SHOW ACTIVITIES
    func showActiviesString(urls: [String], viewcontroller: UIViewController, complention: (() -> Void)?) {
...
//            }
    ...
    func covertToCAF(url: URL, type: ExportFileAV, completion: @escaping((URL) -> Void), failure: @escaping ((String) -> Void)) {
        let documentsPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!
        let name = url.deletingPathExtension().lastPathComponent
        let outputURL = URL(fileURLWithPath: documentsPath)
            .appendingPathComponent("\(ConstantApp.shared.convertFolder)/\(name)")
